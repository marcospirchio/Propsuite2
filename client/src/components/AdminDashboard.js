...
const obtenerReclamosAreasComunesPorEdificio = async (codigoEdificio) => {
  try {
    const response = await fetch(
      `http://localhost:8080/api/reclamos/areas-comunes/${codigoEdificio}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
    ...
    if (response.ok) {
      const data = await response.json();
      console.log("Reclamos filtrados por edificio:", data); // Confirmar que los datos son correctos
      setModalContent(data); // Guardar la respuesta en el estado
      setShowModal("mostrarReclamosAreasComunes"); // Abrir el modal
    } else {
      const errorText = await response.text();
      alert(`Error al obtener reclamos de áreas comunes por edificio: ${errorText}`);
    }
  } catch (error) {
    console.error("Error al obtener reclamos de áreas comunes por edificio:", error);
    alert("Error al conectar con el servidor.");
  }
};

{showModal === "filtrarReclamosPorEdificio" && (
  <div className="modal">
    <div className="modal-content">
      <h2>Filtrar Reclamos por Edificio</h2>
      <input
        type="text"
      ...
      <div className="modal-buttons">
        <button
          ...
          onClick={() => obtenerReclamosAreasComunesPorEdificio(codigoEdificio)}
        >
          Buscar
        </button>
        <button
...
<button
  className="section-button"
  onClick={() => {
    setCodigoEdificio(""); // Limpia el valor previo del código del edificio
    setShowModal("filtrarReclamosPorEdificio");
  }}
>
  Filtrar Reclamos por Edificio
</button>
